...
def detecta_video(classes, emotion_dict, model_humor, model_genero):
    # abrindo webcam
...
def detecta_imagem(classes, emotion_dict, model_humor, model_genero):
    ...
    # aplica face detection
    face, confianca = cv.detect_face(img)
    ...
    for idx, f in enumerate(face):
        ...
        # desenha o retangulo sobre a face detectada
        cv2.rectangle(img, (comecoX,comecoY), (fimX,fimY), (0,255,0), 2)

        # corta a parte detectada
        face_crop = np.copy(img[comecoY:fimY,comecoX:fimX])
        ...
        # escreve a label abaixo do retangulo
        cv2.putText(img, gender_label, (comecoX, comecoY + 20 ),  cv2.FONT_HERSHEY_SIMPLEX,
                    0.6, (0, 0, 255), 2)
        cv2.putText(img, humor_label, (comecoX, comecoY + 40), cv2.FONT_HERSHEY_SIMPLEX,
                    0.6, (0, 0, 255), 2)
...
btmVideo = Button(frm, text='Video', command=lambda: detecta_video(classes, emotion_dict, model_humor, model_genero))
btmImagem = Button(frm, text='Imagem', command=lambda: detecta_imagem(classes, emotion_dict, model_humor, model_genero))
